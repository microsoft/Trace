...
ship_type_to_num = {"C": 1, "B": 1, "R": 2, "S": 2, "D": 2}


# Define BattleshipBoard class
class BattleshipBoard:
    def __init__(
        self, width, height, num_each_type=1, exclude_ships=[], init_with_one_hit=False
    ):
        self.width = width
        self.height = height
        self.ships = {s: ships[s] for s in ships if s not in exclude_ships}
        self.board = create_and_fill_battleship_board(
            width, height, self.ships, num_each_type=num_each_type
        )
            ...
            for row in range(self.height):
                for col in range(self.width):
                    if self.board[row][col] == ship_symbol and not visited[row][col]:
                        ...
                        if (
                            row + 1 < self.height
                            and self.board[row + 1][col] == ship_symbol
                        ):
                            is_vertical = True
                        ...
                        if is_vertical:
                            for i in range(size):
                                current_row = row + i
                                if (
                                    current_row < self.height
                                    and self.board[current_row][col] == ship_symbol
                                ):
                                    part = "body"
                                    if i == 0:
                                        part = "tip"
                                        ...
                                        part = "tail"
                                    ship_positions[ship_symbol].append(
                                        (current_row, col, True, part)
                                    )
                            ...
                            for i in range(size):
                                ...
                                if (
                                    ...
                                    and self.board[row][current_col] == ship_symbol
                                ):
                                    part = "body"
                                    if i == 0:
                                        part = "tip"
