...
class NestedFunctionOptimizer(Optimizer):
    ...
    def nested_summarize(self, node):
        ...
        feedbacks = [self.propagator.aggregate(node.feedback) for node in (parameters | tmp) if (node.trainable or node in tmp)]
    ...
    def _step(self, verbose=False, mask=None, *args, **kwargs) -> Dict[ParameterNode, Any]:
        ...
        problem_instance = self.nested_summarize(self.target)
